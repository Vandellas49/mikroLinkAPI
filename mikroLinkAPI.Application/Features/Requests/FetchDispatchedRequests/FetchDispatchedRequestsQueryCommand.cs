...
{
    public sealed record FetchDispatchedRequestsQueryCommand(
        FilterByFetchDispatchedRequest Filters,
        PageSettings Page, DynamicParameter dynamicfield) : IRequest<Result<Inventory<RequestsModelVM>>>;

    internal sealed class FetchDispatchedRequestsQueryHandler(
        IRequestRepository requestRepository,
        IMapper mapper,
        ICurrentUserService currentUserService) : IRequestHandler<FetchDispatchedRequestsQueryCommand, Result<Inventory<RequestsModelVM>>>
    {
        public async Task<Result<Inventory<RequestsModelVM>>> Handle(FetchDispatchedRequestsQueryCommand request, CancellationToken cancellationToken)
        {
            IQueryable<Request> query = requestRepository.Where(p => true).
                ...
                Where(p => p.CompanyId == currentUserService.UserCompanyId&&p.RequestStatu==(int)RequestStatu.TeamLeader&& p.RequestSiteCompanySerial.Count() > 0);
