...
{
    public sealed class MetarialRequestCommandValidator : AbstractValidator<MetarialRequestCommand>
    {
        readonly IMetarialRepository metarialRepository;
        readonly ICurrentUserService currentUser;

        public MetarialRequestCommandValidator(ICurrentUserService currentUser, IMetarialRepository metarialRepository)
        {
        ...
        private async Task<string> MalzemeKontrol(MetarialRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in model)
            {
                var result = await metarialRepository.Where(p =>
                  ...
                  Select(p => new { p.Defective, p.Sturdy, p.Scrap, talep = p.Component.RequestedMaterial.Where(p => p.Request.RequestStatu == (int)RequestStatu.StockMan && p.Request.CompanyId == currentUser.UserCompanyId).Select(c => new { c.Defective, c.Scrap, c.Sturdy }).ToList() }).
