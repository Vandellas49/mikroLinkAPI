...
{
    public sealed record RequestsReceivedQueryCommand(
        FilterByRequestReceived Filters,
    ...
    internal sealed class RequestsReceivedQueryHandler(
        ...
        IMapper mapper,
        ICurrentUserService currentUserService) : IRequestHandler<RequestsReceivedQueryCommand, Result<Inventory<RequestsModelVM>>>
    {
        public async Task<Result<Inventory<RequestsModelVM>>> Handle(RequestsReceivedQueryCommand request, CancellationToken cancellationToken)
        {
            IQueryable<Request> query = requestRepository.Where(p => true).
                ...
                Include(p => p.Company).
                Include(p => p.Site).
                Where(p => p.ReceiverCompanyId == currentUserService.UserCompanyId&&
                           p.RequestStatu==(int)RequestStatu.TeamLeader&&
                           p.RequestSiteCompanySerial.Count>0);
            ...
            {
                query = query.FilterBy(p => p.Company.Name, request.Filters.RequestDestination.matchMode, request.Filters.RequestDestination.value);
